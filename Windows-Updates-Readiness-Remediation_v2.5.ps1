<#
.SYNOPSIS
    Windows Updates Readiness - Remediation Script v2.5
...
.DESCRIPTION
    ...
    - Recycle Bin cleanup (System-wide)
    ...
    - WU Component repair (DLL re-registration with 24H2/25H2 compatibility)
    - Winsock/Proxy reset
...
.NOTES
    ...
    Changelog v2.5:
    - Aligned with Detection v2.5 cleanup thresholds:
      * Recycle Bin: 5 GB or more triggers cleanup
      * Temp Folders (User + System combined): 1 GB or more triggers cleanup
      * WU Cache: 1 GB or more triggers cleanup
      * DO Cache: 1 GB or more triggers cleanup
      * WER: 1 GB or more triggers cleanup
      * Installer Cache: 1 GB or more triggers cleanup
      * Thumbnail Cache: 1 GB or more triggers cleanup
      * DISM/WinSxS: 10 GB or more triggers cleanup
    - Remediation now runs all cleanup tasks regardless of detection (ensures cleanup completes)

    Changelog v2.4:
    - Fixed Recycle Bin cleanup to target hidden System Recycle.Bin (works for SYSTEM context)
    - Added compatibility for Windows 11 24H2/25H2 (skips problematic DLLs, prevents regsvr32 hangs)
...
Write-Log "==== Windows Updates Readiness Remediation v2.5 ===="
...
function Invoke-RecycleBinCleanup {
    ...
    # Method 1: Direct File System Cleanup (Most effective for SYSTEM context)
    try {
        $RecyclePath = "$env:SystemDrive\`$Recycle.Bin"
        if (Test-Path $RecyclePath) {
            Write-Log "Targeting system-wide Recycle Bin folder: $RecyclePath"
            
            # Get all S-1-5-* folders (User SIDs) inside $Recycle.Bin
            ...
            foreach ($Bin in $UserBins) {
                try {
                    # Empty the contents of each user's bin
                    Get-ChildItem -Path $Bin.FullName -Recurse -Force -ErrorAction SilentlyContinue | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                } catch {}
            }
        ...
        Write-Log "File system cleanup failed: $_" "WARNING"
    }

    # Method 2: Fallback to cmdlet (Only clears SYSTEM's bin usually)
    try {
...
function Repair-WUComponents {
    ...
    # Re-register DLLs (Fixed for Win11 24H2/25H2)
    foreach ($DLL in $WUDLLs) {
        $DLLPath = "$env:SystemRoot\System32\$DLL"
        
        # 1. Check if file exists
        if (Test-Path $DLLPath) {
            # 2. Skip known problematic DLLs on 25H2 that exist but fail to register
            # vbscript/scrrun/mshtml are deprecated/locked on newer builds
            if ($DLL -in @("vbscript.dll", "scrrun.dll", "mshtml.dll", "shdocvw.dll", "browseui.dll")) {
                 Write-Log "Skipping deprecated/locked DLL: $DLL" "INFO"
                 $Global:RemediationStats.DLLsSkipped++
                 continue
            }

            # 3. Use Start-Process to prevent script crash if regsvr32 errors out
            # PassThru is needed to get the exit code
            ...
            if ($Process.ExitCode -eq 0) {
                ...
                Write-Log "Warning: Failed to register $DLL (Exit Code: $($Process.ExitCode))" "WARNING"
            }
        } else {
            # Skip DLL if it doesn't exist
            if ($LegacyDLLs -contains $DLL) {
                Write-Log "Skipped legacy DLL: $DLL (not present)" "INFO"
            } else {
    ...
    Write-Log "Re-registered $($Global:RemediationStats.DLLsReregistered) DLLs (Skipped: $($Global:RemediationStats.DLLsSkipped))"
...
try {
    ...
    Write-Log "DLLs Skipped: $($Global:RemediationStats.DLLsSkipped)"
