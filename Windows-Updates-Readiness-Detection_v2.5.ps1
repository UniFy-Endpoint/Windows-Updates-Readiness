<#
.SYNOPSIS
    Windows Updates Readiness - Detection Script
...
.NOTES
    ...
    - Updated cleanup thresholds based on real-world enterprise experience:
      * Recycle Bin: 2 GB - Catches most cases without being too aggressive
      * Temp Folders (User + System combined): 500 MB - indicates stale files
      * WU Cache: 500 MB - Indicates stuck/failed updates
      * DO Cache: 500 MB - Reasonable for non-peer-caching scenarios
      * WER: 200 MB - Indicates persistent crash issues
      * Installer Cache: 500 MB - Catches orphaned patches
      * Thumbnail Cache: 200 MB - Indicates corruption or excessive regeneration
      * DISM/WinSxS: 8 GB - Typical healthy range is 6-10 GB
...
function Test-RegistryPolicy {
    ...
    foreach ($Name in @('WUServer','WUStatusServer','TargetGroup','TargetGroupEnabled')) {
        ...
        if ($Check.Exists -and $null -ne $Check.Value -and "$($Check.Value)".Length -gt 0) {
            ...
            $Global:Detection.Issues += "Registry: $Name present"
            $Global:Detection.RegistryPolicyIssues = $true
            $Global:Detection.RequiresRemediation = $true
        }
    }
...
function Test-DiskSpaceAndCleanup {
    ...
    # Recycle Bin (Fixed for SYSTEM Context) - Threshold: 2 GB
    try {
    ...
    # User Temp + System Temp (Combined) - Threshold: 500 MB
    ...
    # Windows Update Cache - Threshold: 500 MB
    ...
    # Delivery Optimization Cache - Threshold: 500 MB
    ...
    # Windows Error Reporting - Threshold: 200 MB
    $WERSize = 0
    ...
    # Windows Installer Patch Cache - Threshold: 500 MB
    ...
    # Thumbnail Cache - Threshold: 200 MB (ALIGNED - fixed null handling)
    ...
    # WinSxS / Component Store - Threshold: 8 GB (ALIGNED - now uses Get-FolderSizeGB)
    try {
        ...
        if ($WinSxSGB -ge $CleanupThresholds.WinSxS) {
            ...
            $Global:Detection.Issues += "WinSxS cleanup needed: $WinSxSGB GB"
            $Global:Detection.RequiresRemediation = $true
        } else {
            Write-Log "WinSxS: $WinSxSGB GB (OK)"
        }
    } catch {
        Write-Log "Error checking WinSxS: $_" "WARNING"
    }
