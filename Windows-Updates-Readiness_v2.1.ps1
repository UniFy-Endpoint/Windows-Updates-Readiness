...
.EXAMPLE
    ...
    .\Windows-Updates-Readiness_v2.1.ps1 -DetectOnly
    .\Windows-Updates-Readiness_v2.1.ps1 -AggressiveCleanup
#>
...
Write-Log "===="
Write-Log "Windows Updates Readiness Script v2.1"
...
$Global:DetectionResult = @{
    ...
    DiskSpaceLow = $false
    # Granular detection flags (NEW in v2.1)
    Flags = @{
...
#region --- DISK SPACE & CLEANUP DETECTION (ENHANCED v2.1) ---
...
function Test-DiskSpaceAndCleanupNeeds {
    ...
    try {
        ...
        if ($RecycleBinMB -gt $CleanupThresholds.RecycleBin) {
            ...
            Write-Log "FLAG: Recycle Bin needs cleanup ($RecycleBinMB MB > $($CleanupThresholds.RecycleBin) MB threshold)" "WARNING"
        }

        # 2. Temp Files (User + System)
        ...
        if ($TotalTempMB -gt $CleanupThresholds.TempFiles) {
            ...
            Write-Log "FLAG: Temp files need cleanup ($TotalTempMB MB > $($CleanupThresholds.TempFiles) MB threshold)" "WARNING"
        }
        ...
        if ($WUCacheMB -gt $CleanupThresholds.WUCache) {
            ...
            Write-Log "FLAG: WU cache needs cleanup ($WUCacheMB MB > $($CleanupThresholds.WUCache) MB threshold)" "WARNING"
        }

        # 4. Delivery Optimization Cache (NEW in v2.1)
        ...
        if ($DOCacheMB -gt $CleanupThresholds.DOCache) {
            ...
            Write-Log "FLAG: DO cache needs cleanup ($DOCacheMB MB > $($CleanupThresholds.DOCache) MB threshold)" "WARNING"
        }

        # 5. Windows Error Reporting (NEW in v2.1)
        ...
        if ($TotalWERMB -gt $CleanupThresholds.WER) {
            ...
            Write-Log "FLAG: WER needs cleanup ($TotalWERMB MB > $($CleanupThresholds.WER) MB threshold)" "WARNING"
        }

        # 6. Installer Patch Cache (NEW in v2.1)
        ...
        if ($InstallerCacheMB -gt $CleanupThresholds.InstallerCache) {
            ...
            Write-Log "FLAG: Installer cache needs cleanup ($InstallerCacheMB MB > $($CleanupThresholds.InstallerCache) MB threshold)" "WARNING"
        }

        # 7. Thumbnail Cache (NEW in v2.1)
        ...
        if ($ThumbCacheMB -gt $CleanupThresholds.ThumbnailCache) {
            ...
            Write-Log "FLAG: Thumbnail cache needs cleanup ($ThumbCacheMB MB > $($CleanupThresholds.ThumbnailCache) MB threshold)" "WARNING"
        }

        # 8. WinSxS / Component Store (NEW in v2.1)
        ...
        if ($WinSxSMB -gt $CleanupThresholds.WinSxS) {
            ...
            Write-Log "FLAG: WinSxS/Component Store needs cleanup ($WinSxSMB MB > $($CleanupThresholds.WinSxS) MB threshold)" "WARNING"
        }
        ...
        if (Test-Path $WindowsOldPath) {
            ...
            Write-Log "FLAG: Windows.old exists ($WindowsOldMB MB) - can be removed with -AggressiveCleanup" "WARNING"
        }
...
function Test-WUComponentHealth {
    ...
    # Check critical WU DLLs exist (NEW in v2.1)
    $MissingDLLs = @()
    $CriticalDLLs = @("wuapi.dll", "wuaueng.dll", "wups.dll", "wups2.dll", "wucltux.dll")
    ...
    if ($MissingDLLs.Count -gt 0) {
        ...
        Write-Log "FLAG: Critical WU DLLs missing: $($MissingDLLs -join ', ')" "ERROR"
        $Global:DetectionResult.Issues += "Missing WU DLLs: $($MissingDLLs -join ', ')"
        $Global:DetectionResult.RequiresRemediation = $true
    }

    # Check WU services status (NEW in v2.1)
    $StoppedServices = @()
    ...
    if ($StoppedServices.Count -gt 0) {
        ...
        Write-Log "FLAG: WU services not running: $($StoppedServices -join ', ')" "WARNING"
    }

    # Check Winsock status (NEW in v2.1)
    try {
        $WinsockKey = "HKLM:\SYSTEM\CurrentControlSet\Services\WinSock2\Parameters"
        if (-not (Test-Path $WinsockKey)) {
            $Global:DetectionResult.Flags.WinsockCorrupted = $true
            Write-Log "FLAG: Winsock configuration may be corrupted" "WARNING"
        }
    } catch {}

    # Check proxy configuration (NEW in v2.1)
    try {
        ...
        if ($ProxyOutput -match "Proxy Server" -and $ProxyOutput -notmatch "Direct access") {
            ...
            Write-Log "FLAG: WinHTTP proxy is configured - may affect Windows Update" "WARNING"
        }
    } catch {}
}
#endregion
...
try {
    ...
    # Disk space and cleanup needs detection (ENHANCED v2.1)
    Test-DiskSpaceAndCleanupNeeds

    # WU Component health check (NEW in v2.1)
    ...
    # Display granular flags (NEW in v2.1)
    Write-Log "----" "INFO"
    ...
    $NeedsRemediation = $Global:DetectionResult.RequiresRemediation -or 
                        $Global:DetectionResult.DiskSpaceLow -or
                        $Global:DetectionResult.Flags.WUDLLsMissing -or
                        $Global:DetectionResult.Flags.WUServicesNotRunning -or
                        $Global:DetectionResult.Flags.WinsockCorrupted
